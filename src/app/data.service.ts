...
export class DataService {
  ...
  validateUser(name:string,password:string):Observable<any>{
        const encodedUserAndPass=btoa(`${name}:${password}`);
        const headers=new HttpHeaders().append('Authorization',`Basic `+encodedUserAndPass);
        return this.http.get<any>(environment.restUrl+'/api/basicAuth/validate',{headers:headers});
